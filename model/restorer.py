...
class DynamicPosBias(nn.Module):
    # The implementation builds on Crossformer code
...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def calculate_mask(self, H, W):
        # The implementation builds on Swin Transformer code
    ...
    def forward(self, x, H, W):
        ...
        # window-0 and window-1 on split channels [C/2, C/2]; for square windows (e.g., 8x8), window-0 and window-1
        # can be merged shift in block: (0, 4, 8, ...), (2, 6, 10, ...), (0, 4, 8, ...), (2, 6, 10, ...), ...
...
class Upsample2x(nn.Module):
    def __init__(self, num_feat):
        super().__init__()
        self.conv = torch.nn.Conv2d(num_feat, num_feat, kernel_size=3, stride=1, padding=1)

    def forward(self, x):
        return self.conv(F.interpolate(x, scale_factor=2, mode='nearest'))


class QRE(nn.Module):  # quantised representation enhancement
    def __init__(self, num_feat=180, Cvae=32):
        super().__init__()
        self.num_feat = num_feat
        self.Cvae = Cvae

        self.quant_conv = nn.Sequential(
            nn.Conv2d(Cvae, num_feat, kernel_size=3, padding=1),
    ...
    def forward(self, lr, z):
        """
        Input: lr: (B, H*W, 180), z: (B, 32, 16, 16)
        Output: lr: (B, H*W, 180)
        """
        B, _, C = lr.shape
        dp = self.mlp(self.quant_conv(z)).view(-1, 1, C)
        shortcut = lr * dp
        lr = shortcut + lr
        return lr


class VDAB(nn.Module):  # vector-quantised degradation-aware block
    ...
    def __init__(self, dim, num_heads, reso=64, split_size=[2, 4], shift_size=[1, 2], expansion_factor=4.,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, rg_idx=0, b_idx=0, IBO='upper'):
        super().__init__()
    ...
    def forward(self, x, z, x_size):
        """
        Input: x: (B, H*W, C), x_size: (H, W), z: (B, 32, 16, 16)
        Output: x: (B, H*W, C)
        """
        H, W = x_size
        x = x + self.drop_path(self.attn(self.fuse1(self.norm1(x), z), H, W))
        x = x + self.drop_path(self.ffn(self.fuse2(self.norm2(x), z), H, W))
        return x
...
class DAR(nn.Module):  # degradation-aware restorer
    """
    Args:
    ...
    def forward(self, x, z):
        ...
        """
        x = self.conv_first(x)
        x = self.conv_after_body(self.forward_features(x, z)) + x
...
if __name__ == '__main__':
    ...
    width = 126
    batch_size = 2
    print('lr shape', "height", height, "width", width)
    model = DAR().eval().cuda()
    z = torch.randn((batch_size, 32, 16, 16)).cuda()
    lr = torch.randn((batch_size, 3, height, width)).cuda()
    sr = model(lr, z)
